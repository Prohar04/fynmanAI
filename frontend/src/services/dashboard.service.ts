import { apiFetch, type ApiFetchError } from "@/lib/apiFetch";
...
export type DashboardSessionReport = {
  id: string;
  summary?: string | null;
  strengths?: string[];
  weaknesses?: string[];
...
export const fetchDashboardSessionDetail = async (
  token: string,
  sessionId: string
) => {
  const response = await apiFetch<ApiEnvelope<{ session: DashboardSession }>>(
...
export const fetchDashboardSessionReport = async (
  token: string,
  sessionId: string
) => {
  try {
    const response = await apiFetch<
      ApiEnvelope<{ evaluation: DashboardSessionReport }>
    ...
    if ((error as ApiFetchError)?.status === 404) {
      return null;
    }
    throw error;
  }
...
export const deleteDashboardResource = async (
  token: string,
  resourceId: string
) => {
  await apiFetch<ApiEnvelope<Record<string, unknown>>>(
