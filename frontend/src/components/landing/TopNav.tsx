          ...
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 gap-6">
            {navItems.map((item, index) =>
              item.disabled ? (
                <span
                  key={item.label}
                  className="font-body-sm text-body-sm text-[#c7c4d7]/40 py-1 cursor-not-allowed"
                  title="Coming soon"
                >
                  {item.label}
                </span>
        ...
        <div className="flex flex-col gap-1">
          {navItems.map((item, index) =>
            item.disabled ? (
              <span
                key={item.label}
                className="py-3 px-2 text-base rounded-lg border-b border-[#464554]/30 text-[#c7c4d7]/40 cursor-not-allowed"
              >
                {item.label}
              </span>
            ) : (
