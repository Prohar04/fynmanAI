...
describe('Auth routes', () => {
  ...
  describe('POST /auth/signup', () => {
    ...
    it('rejects signup for an existing passwordless (OAuth) account instead of auto-linking', async () => {
      // Auto-linking a password from an unauthenticated signup request would
      // let anyone take over a Google-linked account just by knowing its
      // email. It must be rejected, and the caller directed to the
      // ownership-verified forgot-password flow instead.
      mockFindUserByEmail.mockResolvedValue({
        id: 'user-1',
        email: 'test@example.com',
        name: 'John Doe',
