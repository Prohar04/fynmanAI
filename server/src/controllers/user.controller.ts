...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    const user = await findUserByVerificationToken(token, 'PASSWORD_RESET');
    if (!user) {
      return sendApiError(res, {
        status: 400,
    ...
    // Hash the new password, update it, and invalidate the reset token
    // in one atomic write so it cannot be replayed.
...
export const verifyEmail = async (req: Request, res: Response) => {
  try {
    ...
    if (!resolvedUserId) {
      const userByToken = await findUserByVerificationToken(
        token,
        'EMAIL_VERIFICATION'
      );
      if (!userByToken) {
    ...
    const verified = await verifyUserEmail(resolvedUserId, token);
    if (!verified) {
      return sendApiError(res, {
        status: 400,
        message: 'Invalid or expired token',
